...
function forumUserAvatar(username, profilePicture) {
    ...
    if (profilePicture) {
        return '<img src="' + escHtml(profilePicture) + '" class="forum-post-avatar-img" alt="' + escHtml(username) + '" onerror="this.style.display=\'none\';this.nextSibling.style.display=\'flex\'">' +
...
function forumBadgesHtml(accountType, accountPermissions) {
    var html = '';
    if (accountType) {
        html += '<span class="forum-user-type-badge" style="' + getAccountTypeBadgeStyle(accountType) + '">' + escHtml(accountType) + '</span>';
    }
    if (accountPermissions && accountPermissions !== '' && accountPermissions !== 'none' && accountPermissions !== null) {
        var pt = Array.isArray(accountPermissions) ? accountPermissions.join(', ') : accountPermissions;
        html += '<span class="forum-user-perms-badge">' + escHtml(pt) + '</span>';
    }
    return html;
}
...
function renderForumPage(section) {
    ...
    var onlineCount = Math.floor(Math.random() * 40) + 12;
    ...
    return '<div class="forum-comment-item" id="forum-comment-' + postIdx + '-' + commentPath + '">' +
        ...
        '<div class="forum-comment-content">' +
            ...
            '<div class="forum-comment-actions">' +
                '<button class="forum-vote-btn forum-comment-vote' + (c._likedByMe ? ' forum-voted-like' : '') + '" onclick="forumCommentVote(' + postIdx + ',\'' + commentPath + '\',\'like\')">' +
                    ...
                    '<span>' + (c.likes || 0) + '</span>' +
                '</button>' +
                '<button class="forum-vote-btn forum-comment-vote' + (c._dislikedByMe ? ' forum-voted-dislike' : '') + '" onclick="forumCommentVote(' + postIdx + ',\'' + commentPath + '\',\'dislike\')">' +
...
function buildForumPostCard(p, idx) {
    ...
    return '<div class="forum-post-card glass-card" id="forumPost-' + idx + '">' +
        ...
        '<div class="forum-post-footer">' +
            '<button class="forum-vote-btn' + (p._likedByMe ? ' forum-voted-like' : '') + '" onclick="forumVote(' + idx + ',\'like\')">' +
                ...
                '<span>' + (p.likes || 0) + '</span>' +
            '</button>' +
            '<button class="forum-vote-btn' + (p._dislikedByMe ? ' forum-voted-dislike' : '') + '" onclick="forumVote(' + idx + ',\'dislike\')">' +
...
function forumVote(idx, type) {
    var p = forumPosts[idx];
    if (!p) return;
    if (type === 'like') {
        if (p._likedByMe) { p._likedByMe = false; p.likes = (p.likes || 1) - 1; }
        else { if (p._dislikedByMe) { p._dislikedByMe = false; p.dislikes = (p.dislikes || 1) - 1; } p._likedByMe = true; p.likes = (p.likes || 0) + 1; }
    } else {
        if (p._dislikedByMe) { p._dislikedByMe = false; p.dislikes = (p.dislikes || 1) - 1; }
        else { if (p._likedByMe) { p._likedByMe = false; p.likes = (p.likes || 1) - 1; } p._dislikedByMe = true; p.dislikes = (p.dislikes || 0) + 1; }
    }
    var section = document.getElementById('forumPageSection');
    if (section) renderForumPage(section);
}

function forumCommentVote(postIdx, commentPath, type) {
    var p = forumPosts[postIdx];
    if (!p || !p.comments) return;
    var c = getCommentByPath(p.comments, commentPath);
    if (!c) return;
    if (type === 'like') {
        if (c._likedByMe) { c._likedByMe = false; c.likes = (c.likes || 1) - 1; }
        else { if (c._dislikedByMe) { c._dislikedByMe = false; c.dislikes = (c.dislikes || 1) - 1; } c._likedByMe = true; c.likes = (c.likes || 0) + 1; }
    } else {
        if (c._dislikedByMe) { c._dislikedByMe = false; c.dislikes = (c.dislikes || 1) - 1; }
        else { if (c._likedByMe) { c._likedByMe = false; c.likes = (c.likes || 1) - 1; } c._dislikedByMe = true; c.dislikes = (c.dislikes || 0) + 1; }
    }
    var section = document.getElementById('forumPageSection');
    if (section) renderForumPage(section);
}
...
function submitForumComment(idx) {
    var input = document.getElementById('forumCommentInput-' + idx);
    if (!input || !input.value.trim()) return;
    if (!currentUser) return;
    var now = new Date();
    var timeStr = now.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) + ' · ' + String(now.getHours()).padStart(2, '0') + ':' + String(now.getMinutes()).padStart(2, '0');
    var p = forumPosts[idx];
...
function submitForumReply(postIdx, commentPath) {
...
function submitForumPost() {
    ...
    var userPic = '';
    if (currentUser) {
        userPic = currentUser.profile_picture_base64 ? 'data:image/png;base64,' + currentUser.profile_picture_base64 : (currentUser.profile_picture || '');
    }
    forumPosts.unshift({
        username: (currentUser && currentUser.username) || 'User',
        account_type: (currentUser && currentUser.account_type) || null,
        account_permissions: (currentUser && currentUser.account_permissions) || null,
        profile_picture: userPic || null,
        title: title.trim(),
        body: body.trim(),
        time: timeStr,
...
function submitUserReview() {
    ...
    if (container) { container.dataset.rendered = ''; showReviewsPage(); }
...
function showReviewsPage() {
    ...
    var container = document.getElementById('reviewsPageContainer');
    if (!container) return;
    if (container.dataset.rendered === '1' && userWrittenReviews.length === 0) return;
    container.dataset.rendered = '1';

    var reviews = getReviewsData().concat(userWrittenReviews);
    ...
    reviews.forEach(function(r, idx) {
        ...
        var avatarHtml = r.profile_picture
            ? '<img src="' + escHtml(r.profile_picture) + '" class="review-avatar-img" alt="' + escHtml(r.username) + '" onerror="this.style.display=\'none\';this.nextSibling.style.display=\'flex\'">' +
              '<div class="review-avatar" style="display:none;">' + escHtml(initials) + '</div>'
            : '<div class="review-avatar">' + escHtml(initials) + '</div>';

        var typeBadgeHtml = r.account_type
            ? '<span class="forum-user-type-badge" style="' + getAccountTypeBadgeStyle(r.account_type) + '">' + escHtml(r.account_type) + '</span>'
            : '';
        var permsBadgeHtml = (r.account_permissions && r.account_permissions !== '' && r.account_permissions !== 'none')
            ? '<span class="forum-user-perms-badge">' + escHtml(Array.isArray(r.account_permissions) ? r.account_permissions.join(', ') : r.account_permissions) + '</span>'
            : '';
